...
- **Composer** (`.composer`): the book view is notes-first — a slim header (`Shelf` link · status select · Edit/Remove), title/author, the page-set, then a thin progress strip. Below it the composer sits **always open** as the ledger's next, empty ruled row: a ¶ folio with a bare mono page input (defaults to the current page, walks to the next free one after a save), one serif textarea that auto-grows, and a faint ✚ Photo / Save pair that rises to full opacity on focus. No box, no labels, no hint copy. Enter saves, `⇧↵` starts a new line; pasting or picking a photo stages thumbs inline.
...
**The note is the progress.** Saving a note at page N moves the current page to N. One source of truth (the page number), two entry points (header page-set, composer page chip) — no separate "mark as read" button, because both acts are the same act. Compose flow is friction-free: the composer is always open at the next free page, Enter saves, and the ledger re-renders beneath it.

## State & persistence

Single key `readnotes.v1` → `{ books: [{ id, title, author, totalPages, currentPage, status, notes:[{page,text,images,updatedAt}], createdAt, updatedAt }] }`. Images downscaled to 1200px/WebP before being stored as data URLs; a soft warn fires past ~4.2MB, a hard `QuotaExceededError` guard toasts without corrupting state. JSON export (dated download) / import (merge by id) in the appbar. Routes: `#/` shelf, `#/book/:id`.
